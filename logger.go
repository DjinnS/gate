...
func NewLogChannel(startTime time.Time, channel ssh.Channel, username string, destination string, logDir string) *LogChannel {
	return &LogChannel{
		StartTime:     startTime,
		UserName:      username,
...
func (l *LogChannel) SyncToFile(remote_name string) error {
	...
	filepath := fmt.Sprintf("%s/%s/%d/%d/%d", "logs/", l.Destination, l.StartTime.Year(), l.StartTime.Month(), l.StartTime.Day())
	err = os.MkdirAll(filepath, 0750)
	if err != nil {
		return fmt.Errorf("Unable to create required log directory (%s): %s", filepath, err)
	}
	filename := filepath + "/" + fmt.Sprintf("%s.%s", l.UserName, l.StartTime.Format("150405"))
	//filename := "logs/" + fmt.Sprintf("ssh_log_%s", l.StartTime.Format(time.RFC3339))

	l.logMutex.Lock()

	l.fd, err = os.OpenFile(filename, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0640)
	if err != nil {
		return err
	}
	...
	l.fd_ttyrec, err = os.OpenFile(filename+".ttyrec", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0640)
	if err != nil {
		return err
	}
	...
	l.fd_req, err = os.OpenFile(filename+".req", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0640)
	if err != nil {
		return err
	}
